...
export const vincularExpedientePendiente = async (
  id: number,
  body: { expediente_numero: string; expediente_anio: number }
) => {
  const { data } = await apiClient.put(
    `/actuaciones/pendientes/acta/${id}/expediente`,
    body
  );
  return data;
};
// api
export const getPendientesVincActa = async (): Promise<IActuacionListado[]> => {
  const { data } = await apiClient.get("/actuaciones/pendientes-vinc-acta");
  return data;
};

export const createExpedienteDesdeActa = async (
  actuacionId: number,
  body: { expediente_numero: string; expediente_anio: number }
): Promise<IActuacionListado> => {
...
export const getPendientesVincOficio = async (): Promise<IActuacionListado[]> => {
  const { data } = await apiClient.get("/actuaciones/pendientes-vinc-oficio");
  return data;
};
export const getPendientesNotificacion = async (): Promise<IActuacionListado[]> => {
  const { data } = await apiClient.get("/actuaciones/pendientes-notificacion");
  return data;
};
export type CardsResumen = {
  actuaciones: number;
...
export const getCardsActuaciones = async (): Promise<CardsResumen> => {
  const { data } = await apiClient.get("/actuaciones/cards");
  return data;
};
